...
def readData(video_name, src_csv_dir, data_type=["spatial","temporal"]):
    spatial_dir=src_csv_dir+"/spatial/"
    temporal_dir=src_csv_dir+"/temporal/"
    data=[]
    for dtype in data_type:
        if dtype=="spatial":
            df=pd.read_csv(spatial_dir+video_name+".csv")
        elif dtype=="temporal":
            df=pd.read_csv(temporal_dir+video_name+".csv")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="rescale feature to csv")
    parser.add_argument("source_csv_dir")
    parser.add_argument("out_csv_dir")
    parser.add_argument("video_info_file")
    parser.add_argument("--out_json_file", type=str, default='./tsn_anet_anno_100.json')

    args = parser.parse_args()

    src_csv_dir = args.source_csv_dir
    out_csv_dir = args.out_csv_dir
    out_json_file = args.out_json_file

    videoDict=getDatasetDict(args.video_info_file)
    ...
    for videoName in videoNameList:
        ...
        # try:
        data=readData(videoName,src_csv_dir)
        # except:
        ...
        scale_len = 100 # len(data) # max( len(data) , 100 )
        ...
        if not os.path.isdir(out_csv_dir):
            print("creating folder: "+ out_csv_dir)
            os.makedirs(out_csv_dir)
        outDf.to_csv(out_csv_dir+"/"+videoName+".csv",index=False)

    outfile=open(out_json_file,"w")
    # outfile = codecs.open(out_json_file, "w", 'utf-8')
    json.dump(videoDict, outfile, indent=1, ensure_ascii=False)
    # outfile.write(json.dumps(videoDict, ensure_ascii=False))
    outfile.close()